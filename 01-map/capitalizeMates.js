...
const capitalizeMates = (mates)=>{
    ...
    return mates.map((name)=>{
        ...
        return name.split(' ').map((character) => { // split mate and map each character
            return character.charAt(0).toUpperCase() + character.slice(1).toLowerCase(); //capitalize first character and lowercase the rest starting from index 1--> slice(1)
